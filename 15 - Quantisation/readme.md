...
## Thread organisation
  ...
  Within each block:
    ...
    Step 4: Each thread quantises its elements<br/>

## Results on NVIDIA T4 (Google colab)
  Matrix: 64 channels x 256 features<br/>
  FP32 size: 64KB → INT8 size: 16KB<br/>

  Quantisation time:   105.7711 ms (one-time offline)<br/>
  Dequantisation time: 0.0287 ms   (every forward pass)<br/>

  Max error:      0.3936% of max weight value<br/>
  Mean error:     0.1940%<br/>
  Compression:    3.94x (scale factor overhead explains gap from 4x)<br/>
...
## Why loading speedup is small on this test :
  ...
  At DRAM : INT8 moves half the data so 2x throughput improvement<br/>
...
## How to compile and run
  nvcc -O2 -o quantisation_int8 quantisation_int8.cu -lm && ./quantisation_int8<br/>
